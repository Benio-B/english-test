...
let numberQuestionsToDisplay = allQuestions.length;
...
function filterQuestionsByCategory(category) {
  ...
  numberQuestionsToDisplay = questions.length;
  showNewQuestion();
}
...
function showNewQuestion() {
  ...
  const numberRemainingQuestions =
    numberQuestionsToDisplay - questions.length + 1;
  categoryElement.textContent =
    currentQuestion.explicitCategory +
    ` (${numberRemainingQuestions}/${numberQuestionsToDisplay})`;

  languageIndicatorElement.textContent = isShowingEnglishQuestion
    ? "English:"
    : "Français:";
  languageIndicatorElement.classList.remove("language-fr");
  languageIndicatorElement.classList.remove("language-en");
  languageIndicatorElement.classList.add(
    isShowingEnglishQuestion ? "language-en" : "language-fr",
  );

  questionElement.textContent = isShowingEnglishQuestion
...
function showAnswer() {
  ...
  answerElement.textContent = isShowingEnglishQuestion
    ? currentQuestion.french
    : currentQuestion.english;
  if (!isShowingAnswer) {
